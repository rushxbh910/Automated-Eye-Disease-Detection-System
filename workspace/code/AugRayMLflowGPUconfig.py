...
print(f"Classes: {train_dataset_original.classes}, Total Classes: {num_classes}")
...
def train_lightning_model():
    # Training parameters
    experiment_name = "DenseNet121_Eye_Classification_Augmented"
    epochs = 20
    learning_rate = 0.001
    ...
    if use_mlflow:
        try:
            active_run = mlflow.start_run(run_name=f"densenet121_augmented_run_{time.strftime('%Y%m%d_%H%M%S')}")
            ...
            use_mlflow = False
            active_run = DummyContextManager()
    else:
        active_run = DummyContextManager()
    ...
    with active_run:
        ...
        mlflow_callback = MLflowLoggingCallback(use_mlflow=use_mlflow)
        
        # Initialize trainer with callbacks
        trainer = pl.Trainer(
            max_epochs=epochs,
            callbacks=[early_stop, metrics_callback, mlflow_callback],
            enable_progress_bar=True,
            log_every_n_steps=5
        )
        
        # Train the model
        print("\nTraining DenseNet121 with Data Augmentation...\n")
        trainer.fit(model, train_loader, val_loader)
        
        # Save the model in .pth format
        model_path = '/mnt/data/densenet121_augmented_model.pth'
        torch.save(model.state_dict(), model_path)
        print(f"Model saved as densenet121_augmented_model.pth")
        ...
        plot_accuracy_and_loss(train_losses, val_losses, train_accs, val_accs, use_mlflow)
        ...
        plot_confusion_matrix(all_val_labels, all_val_preds, class_names, use_mlflow)
        plot_per_class_accuracy(all_val_labels, all_val_preds, class_names, use_mlflow)
        val_accuracy = compute_classification_metrics(all_val_labels, all_val_preds, num_classes, class_names, use_mlflow)
        ...
        test_accuracy = compute_classification_metrics(all_test_labels, all_test_preds, num_classes, class_names, use_mlflow=False)
        ...
        if use_mlflow:
            try:
                ...
                print(f"Error logging test metrics to MLflow: {e}")
            
            print("MLflow run completed successfully")
    
    return model, trainer, metrics_callback, test_accuracy

# Main execution
if __name__ == "__main__":
    # Train the Lightning model
    model, trainer, metrics_callback, test_accuracy = train_lightning_model()
